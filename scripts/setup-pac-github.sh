#!/usr/bin/env bash
#
# Setup script for Pipelines-as-Code (PAC) with GitHub webhook
#
# This script:
# 1. Creates a kind cluster (optional, if not already exists)
# 2. Installs Tekton Pipelines
# 3. Installs Pipelines-as-Code (remote release by default)
# 4. Sets up gosmee for webhook forwarding
# 5. Creates namespace, secrets, and Repository CR for GitHub project
#
# Usage:
...
#   GITHUB_TOKEN="ghp_xxx" WEBHOOK_SECRET="mysecret123" \
#   GITHUB_REPO_URL="https://github.com/infernus01/git-repo-1" \
#   ./setup-pac-github.sh
#
# To deploy PAC from local source (requires ko, may have compatibility issues):
#   PAC_DEPLOY_MODE=local ./setup-pac-github.sh
#
...
check_prerequisites() {
    ...
    if [[ -z "${GITHUB_TOKEN}" ]]; then
        ...
        echo "  ./setup-pac-github.sh"
        exit 1
    fi
    log_info "Using GitHub token: ${GITHUB_TOKEN:0:10}..."
...
EOF
    ...
    log_info "Webhook URL for GitHub: ${GOSMEE_URL}"
...
create_repository_cr() {
    ...
    log_info "Creating Repository CR for ${GITHUB_REPO_URL}..."
...
print_github_webhook_instructions() {
    log_step "GitHub Webhook Configuration Instructions"
    ...
    echo "   ${GITHUB_REPO_URL}/settings/hooks/new"
    echo ""
...
cleanup() {
    ...
    echo ""
    echo "  # Remove the GitHub webhook manually from:"
    echo "  ${GITHUB_REPO_URL}/settings/hooks"
    echo ""
}
...
main() {
    echo ""
    echo "╔═══════════════════════════════════════════════════════════════════════╗"
    echo "║  Pipelines-as-Code Setup for GitHub                                   ║"
    echo "╚═══════════════════════════════════════════════════════════════════════╝"
    echo ""
